...
/*****************  CONFIGURATION BITS – FRC + PLL16  *****************/
/* 1) Oscilador primario: FRC con PLL ×16
 * 2) Oscilador secundario/Failsafe: FRC (sin cristal externo)
 * 3) Sin cambio dinámico de reloj ni monitor de reloj         */
#pragma config FPR     = FRC_PLL16     // Primary Osc = FRC ×16 PLL
#pragma config FOS     = FRC           // Oscillator Selection at Reset = FRC
...
/* Reloj derivado */
#define _CRYSTAL_WORK   (7370000UL*16) //7.37Mhz
#define FCY             (_CRYSTAL_WORK/4)          // Hz (usado por libpic30)
